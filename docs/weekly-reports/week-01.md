...
A couple of findings from these results.
- For a 3B parameter model in FP32, the memory usage is calculated 
by taking the number of parameters, 3B, and multiplying by 4 bytes (32 bits), which results in approximately 12.86 GB of memory usage.
- For a 3B parameter model in FP16, the memory usage is calculated 
by taking the number of parameters, 3B, and multiplying by 2 bytes (16 bits), which results in approximately 6.43 GB of memory usage.
